...
if (isset($_REQUEST['export'])) {
    ...
    // Create with headers
    $csv = "Summary,Worklist ID,Amount,Date of Payment,Payee,Status,ID\n";
	...
	header('Content-Disposition: attachment; filename="Paypal Report.csv"');
	echo $csv;
} else {
...
function getReport() {
	...
	for ($i = 1; $rtQuery && $row=mysql_fetch_assoc($rtQuery); $i++) {
        ...
        if ($export) {
            $ex_report[] = array($row['summary'], $row['worklist_id'], $row['payment_gross'], $row['paid_date'], $row['nickname'], $row['status'], $row['id']);
        }
	}
	
    if ($export) return $ex_report;
	else return $report;
}
