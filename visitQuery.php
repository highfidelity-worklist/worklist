...
if ($jobid > 0) {
    $results = getJobResults($jobid, $token, $ids);
} elseif ($jobid === 0) {
    $results = getAllJobResults($token, $ids);
} 
if (!isset($results)) {
    returnError('invalid job id supplied');
} else {
    if(!isset($results['error'])) {
        if ($jobid === 0) {
            $data = parseItems($results['result']);
        } else {
            $data = parseItem($results['result']);
        }
        returnValues($data);
    } else {
        returnError($results['error']);
    }
}
...
function getJobResults($jobid, $token, $ids) {
    ...
    return getResults($query, $token, $ids);
}
