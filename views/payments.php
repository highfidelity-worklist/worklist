...
class PaymentsView extends View {
    ...
    public function paymentsTableRows() {
        ...
        foreach ($payee_totals as $payee) {
            $ret .=
                '<tr><td><input type="checkbox" name="'.$payee["mechanic_id"].'fees" onclick="javascript:toggleCBGroup(\'fees'.$payee["mechanic_id"].'\', this);" rel="0" /></td>' .
                '<td colspan="4" align="left"><a href="javascript:void(0);" onclick="toggleVis(\'indfees'.$payee["mechanic_id"].'\')">'.$payee["mechanic_nick"].'</a></td>' .
