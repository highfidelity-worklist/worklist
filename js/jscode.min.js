...
if($.ui){$.widget('ui.toaster',{_init:function(){var self=this;var wrapper='#ui-toaster-'+self.options.position;if(!$(wrapper).get(0)){$('<div />').attr('id','ui-toaster-'+self.options.position).appendTo('body');}
...
$.browser.version=navigator.userAgent.toLowerCase().match(/.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/)[1];})(jQuery);jQuery.cookie=function(name,value,options){if(typeof value!='undefined'){options=options||{};if(value===null){value='';options.expires=-1;}
...
$target.html(this._generateHTML(inst));$target[(this._get(inst,'isRTL')?'add':'remove')+'Class']('countdown_rtl');var onTick=this._get(inst,'onTick');if(onTick){var periods=inst._hold!='lap'?inst._periods:this._calculatePeriods(inst,inst._show,new Date());onTick.apply(target,[periods]);}
...
var compact=this._get(inst,'compact');var layout=this._get(inst,'layout');var labels=(compact?this._get(inst,'compactLabels'):this._get(inst,'labels'));var timeSeparator=this._get(inst,'timeSeparator');var description=this._get(inst,'description')||'';var showCompact=function(period){var labelsNum=$.countdown._get(inst,'compactLabels'+periods[period]);return(show[period]?periods[period]+
(labelsNum?labelsNum[period]:labels[period])+' ':'');};var showFull=function(period){var labelsNum=$.countdown._get(inst,'labels'+periods[period]);return(show[period]?'<span class="countdown_section"><span class="countdown_amount">'+
periods[period]+'</span><br/>'+
(labelsNum?labelsNum[period]:labels[period])+'</span>':'');};return(layout?this._buildLayout(inst,show,layout,compact):((compact?'<span class="countdown_row countdown_amount'+
...
value='0000000000'+value;return value.substr(value.length-len);},_determineShow:function(inst){var format=this._get(inst,'format');var show=[];show[Y]=(format.match('y')?'?':(format.match('Y')?'!':null));show[O]=(format.match('o')?'?':(format.match('O')?'!':null));show[W]=(format.match('w')?'?':(format.match('W')?'!':null));show[D]=(format.match('d')?'?':(format.match('D')?'!':null));show[H]=(format.match('h')?'?':(format.match('H')?'!':null));show[M]=(format.match('m')?'?':(format.match('M')?'!':null));show[S]=(format.match('s')?'?':(format.match('S')?'!':null));return show;},_calculatePeriods:function(inst,show,now){inst._now=now;inst._now.setMilliseconds(0);var until=new Date(inst._now.getTime());if(inst._since){if(now.getTime()<inst._since.getTime()){inst._now=now=until;}
...
else{$.countdown._attachCountdown(this,options);}});};$.countdown=new Countdown();})(jQuery);(function($){var height=$.fn.height,width=$.fn.width;$.fn.extend({height:function(){if(this[0]==window)
...
complete();if(settings(this).fixPNG)helper.parent.unfixPNG();}})(jQuery);(function($){var
...
var pollingInterval=1000;var speakerPollingInterval=10000;var mouseTrigger=false;var pendingMessages=0;var pendingUpdate=false;var retries=0;var retryMessage='';var queryStr='';var inThePresent=true;var months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];var scrollviewHeight,scrollviewTop;var scrollbarBoxHeight,scrollbarThumbHeight,scrollbarThumbMin,scrollbarThumbMax,scrollbarRange;var speakerHeight=0;var speakerListHeight=0;var oldSpeakerList=new Array();var systemMsgs='all';var systemDrawer=false;var penalized=false;var suspended=false;var last_private='';var current_ajax=null;var shiftPressed=false;var playSoundOnNewMessage=true;var displayPopupOnNewMessage=false;var popupHasBeenDisplayed=false;var chatSound,systemSound,pingSound,botSound;var newMessageCount=0;var focus=true;$.sendingMessage=0;$(window).focus(function setFocused(){focus=true;newMessageCount=0;document.title="Journal";if(pendingMessages==0)$('#entries-pending').hide();else inThePresent=false;});$(window).blur(function setNotFocused(){focus=false;});(function(){var textSelection={getSelection:function(){var e=this.jquery?this[0]:this;return(('getSelection'in window&&function(){return window.getSelection().toString();})||('getSelection'in document&&function(){return document.getSelection().toString();})||('selectionStart'in e&&function(){var l=e.selectionEnd-e.selectionStart;return e.value.substr(e.selectionStart,l);})||('selection'in document&&function(){return document.selection.createRange().text;})||function(){return'';})();}};jQuery.each(textSelection,function(i){jQuery.fn[i]=this;});})();function alertNewMessage(ping){if(!focus)
{newMessageCount++;document.title="Journal ("+newMessageCount+")";}
if(playSoundOnNewMessage==true){var lastEntry=$("div.entry:has(h2):last");if(ping&&pingSound){pingSound.play();}else if((lastEntry.hasClass('svn')||lastEntry.hasClass('worklist')||lastEntry.hasClass('sendlove'))&&(systemSound==true)){systemSound.play();}else if(lastEntry.hasClass('bot')){if(botSound){botSound.play();}}else{if(chatSound){chatSound.play();}}}
if(displayPopupOnNewMessage==true&&popupHasBeenDisplayed==false){popupHasBeenDisplayed=true;alert('New Message!');popupHasBeenDisplayed=false;displayPopupOnNewMessage=false;}}
function toggleAudio(){if(playSoundOnNewMessage==true){playSoundOnNewMessage=false;document.getElementById("audioSwitch").src="images/Mute.icon.gif";return"off";}else{playSoundOnNewMessage=true;document.getElementById("audioSwitch").src="images/Volume.icon.gif";return"on";}}
function AudioOn(){document.getElementById("chataudioon").checked=true;document.getElementById("systemaudioon").checked=true;document.getElementById("pingaudioon").checked=true;document.getElementById("botaudioon").checked=true;if(!chatSound)return;chatSound.unmute();systemSound.unmute();pingSound.unmute();botSound.unmute();setVolumeIcon();hideAudioPopup();}
function AudioOff(){chatSound.mute();systemSound.mute();pingSound.mute();botSound.mute();document.getElementById("chataudiooff").checked=true;document.getElementById("systemaudiooff").checked=true;document.getElementById("pingaudiooff").checked=true;document.getElementById("botaudiooff").checked=true;setVolumeIcon();hideAudioPopup();}
function ChatAudioOn(){chatSound.unmute();setVolumeIcon();hideAudioPopup();}
function ChatAudioOff(){chatSound.mute();setVolumeIcon();hideAudioPopup();}
function SystemAudioOn(){systemSound.unmute();setVolumeIcon();hideAudioPopup();}
function SystemAudioOff(){systemSound.mute();setVolumeIcon();hideAudioPopup();}
function PingAudioOn(){pingSound.unmute();setVolumeIcon();hideAudioPopup();}
function PingAudioOff(){pingSound.mute();setVolumeIcon();hideAudioPopup();}
function BotAudioOn(){botSound.unmute();setVolumeIcon();hideAudioPopup();}
function BotAudioOff(){botSound.mute();setVolumeIcon();hideAudioPopup();}
function setVolumeIcon(){if($("#chataudioon:checked").val()&&$("#systemaudioon:checked").val()&&$("#pingaudioon:checked").val()&&$("#botaudioon:checked").val()){$("#audioSwitch").css('background-position','-105px -118px');}else if($("#chataudiooff:checked").val()&&$("#systemaudiooff:checked").val()&&$("#pingaudiooff:checked").val()&&$("#botaudiooff:checked").val()){$("#audioSwitch").css('background-position','-179px -118px');}else{$("#audioSwitch").css('background-position','-140px -118px');}}
function hideAudioPopup(){if($('div#Audiosettings').is(":visible"))
$('div#Audiosettings').hide('drop',{direction:'left'},500);}
function togglePopup(){if(displayPopupOnNewMessage==true){displayPopupOnNewMessage=false;return"will not";}else{displayPopupOnNewMessage=true;return"will";}}
function sendEntry(){var msg=$("#message-pane").val();if(msg.length>1||(msg.length==1&&msg.charCodeAt(0)!=10&&msg.charCodeAt(0)!=13)){retries=0;retryMessage=msg;sendEntryRetry();}
$("#message-pane").val("");setLocalTypingStatus(IDLE);}
function sendEntryRetry(){if(current_ajax)current_ajax.abort();clearInterval($.timer['getLatestEntries']);$.ajax({type:"GET",url:'aj.php',data:{'what':'send','message':retryMessage,'tries':retries,'lastid':lastId,'lasttouched':lastTouched,'filter':systemMsgs,'last_private':last_private},dataType:'json',success:function(json){if(json.scope=='#private'){last_private=json.bot;$('#entries').append(json.html);finishUpdate('100%',true);}
else
{if(!inThePresent){showLatest();}else{updateNewMessages(json,true);}}},error:function(xhdr,status,err){if(retries++<3){setTimeout('sendEntryRetry()',retries*250);}else{alert("Error sending entry.");}},complete:function()
{clearInterval($.timer['getLatestEntries']);$.timer['getLatestEntries']=setInterval('getLatestEntries()',pollingInterval);$.polling=false;$.sendingMessage--;}});}
function showLatest(){var entries=$("#entries");$("#query").val('');queryStr='';entries.empty();pendingMessages=0;inThePresent=true;$.polling=true;$.pollingLatest=true;if(current_ajax)current_ajax.abort();clearInterval($.timer['getLatestEntries']);$.timer['getLatestEntries']=setInterval('getLatestEntries()',pollingInterval);$('.scroll-pane-throbber').show();current_ajax=$.ajax({url:'aj.php',data:{'what':'latest_longpoll','count':50,'filter':systemMsgs},dataType:'json',success:function(json){$('.scroll-pane-throbber').hide();$('#entries-pending').hide();lastId=json.lastId;currentTime=parseInt(json.time);firstDate=json.firstDate;lastDate=json.lastDate;entries.append(json.html);last_private=null;updateTypingIndicators(json.typingstatus);if(systemDrawer){var drawer=$("#system-drawer");drawer.empty();drawer.append(json.system_html);$('#system-drawer-container').scrollTo('100%');}
finishUpdate('100%',false);},complete:function(){$.polling=false;$.pollingLatest=false;clearInterval($.timer['getLatestEntries']);$.timer['getLatestEntries']=setInterval('getLatestEntries()',pollingInterval);}});}
function getLatestEntries(timeout)
{if(!(timeout|0))timeout=30;if($.polling==true)return;$.polling=true;current_ajax=$.ajax({url:'aj.php',data:{'what':'latest_longpoll','timeout':timeout,'lastid':lastId,'lasttouched':lastTouched,'filter':systemMsgs,'last_private':last_private},dataType:'json',timeout:timeout*1000*2,success:function(json){updateTypingIndicators(json.typingstatus);if($.pollingLatest==true||$.sendingMessage>0)return;updateNewMessages(json,false);},complete:function(){$.polling=false;}});applyMessagePruning();}
function updateNewMessages(json,redraw){if(json.lasttouched)lastTouched=json.lasttouched;if(json.updates==0)return;currentTime=parseInt(json.time);ping=(json.botdata&&json.botdata.ping)?json.botdata.ping:false;if(json.lastId&&json.lastId>0)lastId=json.lastId;if(json.count>0){last_private=null;if(inThePresent){$('#entries-pending').hide();lastDate=json.lastDate;if(json.html){$('#entries').append(json.html);}else{for(entry in json.newentries){$('#entries').append(formatMessage(json.newentries[entry]));}}
finishUpdate('100%',true,ping);}else{pendingMessages+=json.count;$('#entries-pending-count').text(pendingMessages+' unread message'+(pendingMessages>1?'s':'')+'...');$('#entries-pending').show();alertNewMessage(ping);}}
if(json.system_count>0){if(json.system_html){$('#system-drawer').append(json.system_html);}else{for(entry in json.newsystementries){$('#system-drawer').append(formatMessage(json.newsystementries[entry]));}}
$('#system-drawer-container').scrollTo('100%');enlivenEntries();}}
function formatMessage(entry){if(document.getElementById('entry-'+entry.id))return('');var d1=$('<div class="entry">');d1.attr('id','entry-'+entry.id);if(entry.custom_class)d1.addClass(entry.custom_class);var d2=$('<div class="entry-text">')
d2.html(entry.entry_text);if(entry.entry_type=='basic')d1.append(d2);else{var h2=$('<h2>');if(entry.nickname!=entry.author)h2.addClass('other');var span1='<span class="entry-author" '+entry.func+'>'+entry.author+'</span>';var span2='<span class="entry-date" data="'+entry.time+'" title="'+entry.time_title+'">';span2+=entry.relative_time+'</span>';var spans=span1+span2;h2.append(spans);d1.append(h2).append(d2);}
return(d1);}
function getLatestSpeakers(timeout)
{if($.speakerPolling==true)return;$.speakerPolling=true;$.ajax({url:'aj.php',data:{'what':'speakerList'},dataType:'json',timeout:timeout*1000*2,success:function(json){fillSpeakerList(json.speakers);currentTime=parseInt(json.time);updateRelativeTimes();},complete:function(){$.speakerPolling=false;}});}
function applyPopupBehavior(){var entries=$("#entries");entries.find("a.attachment").not(".attached").each(function(){$(this).addClass("attached");var imgURL=$(this).attr('href');var myID=imgURL.substring(imgURL.lastIndexOf('=')+1,imgURL.length);$(this).wrap('<div title="'+imgURL+'" id="innerLinkAttach_'+myID+'" style="display:table-cell;padding:3px 0px 3px 0px;vertical-align:middle" />');var iconTag='';iconTag+='<div title="'+imgURL+'" style="display:table-cell;padding:3px 2px 3px 0px;vertical-align:middle">';iconTag+='<img src="images/gif.gif" id="attach_img" height="18" width="18" alt="Attachment"/>'
iconTag+='</div>'
$('div#innerLinkAttach_'+myID).before(iconTag)
$(this).html('Attachment #'+myID);$(this).unbind().click(function(evt){evt.preventDefault();var dialogUrl=$(this).attr('href');$('<img src="'+dialogUrl+'" title="Preview">').dialog({modal:false,width:'auto',height:'auto',open:function(evt){$(evt.target).parent().css({'visibility':'hidden'});storeCursorStatus=new Array();$('*').each(function(){if($(this).css('cursor')!='auto')
storeCursorStatus.push([$(this),$(this).css('cursor')]);});$('*:visible').css('cursor','wait');window.imageFiredDialogRedim=[false,evt.target];$(evt.target).load(function(){window.imageFiredDialogRedim[0]=true;var origWidt=parseInt(this.naturalWidth);var origHeig=parseInt(this.naturalHeight);if(!origWidt||!origHeig){var origWidt=parseInt(this.width());var origHeig=parseInt(this.height());}
var padding=10;ratio=Math.min(($('body').innerWidth()-padding*2)/origWidt,($('body').innerHeight()-padding*2)/origHeig);var zoom='';if(ratio<1){$(this).css({'width':origWidt*ratio,'height':origHeig*ratio});zoom='('+Math.round(ratio*100)+'%)';}
$(this).prev('div').append('<span id="dialogZoom" style="margin-left:10px;">'+zoom+'</span>');var dialog=$(this).parent()
dialog.css({'top':(function(){return(($('body').innerHeight()-dialog.height())/2)+40;})(),'left':(function(){return(($('body').innerWidth()-dialog.width())/2);})()});$('*').css('cursor','auto');$.each(storeCursorStatus,function(i,v){v[0].css('cursor',v[1]);});dialog.css({'visibility':'visible'});})
window.imageFiredDialogRedim[3]=setInterval(function(){if(!window.imageFiredDialogRedim[0]){$(window.imageFiredDialogRedim[1]).trigger('load');clearInterval(window.imageFiredDialogRedim[3]);}},1000);},resizeStart:function(){$(this).parent().find('#dialogZoom').html('');},dragStop:function(evt){var dialog=$(evt.target);if(dialog.position().top<5)
dialog.css({'top':(function(){return(($('body').innerHeight()-dialog.height())/2)+40;})(),'left':(function(){return(($('body').innerWidth()-dialog.width())/2);})()});}});});});}
function combineSpeakerLists(oldList,newList){var user_id;var combinedList=new Array();for(var i=0;i<oldList.length;i++){user_id=oldList[i][0];combinedList[user_id]=new Array(0,i);}
var changes=Math.abs(oldList.length-newList.length);for(var i=0;i<newList.length;i++){user_id=newList[i][0];if(combinedList[user_id]!=undefined){if(combinedList[user_id][1]!=i)changes++;combinedList[user_id][0]=1;combinedList[user_id][1]=i;}else{changes++;combinedList[user_id]=new Array(2,i,newList[i]);}}
return new Array(changes,combinedList);}
var scrollInterval,scrollDelay,scrollDelta;function doAutoScroll(){if(scrollDelay-->0)return;var height=$('#entries').outerHeight();if(height<=scrollviewHeight)return;var entriesTop=$('#entries').position().top;entriesTop=entriesTop+scrollDelta;if(entriesTop>0){entriesTop=0;getEntriesNear(true,true);}else if(entriesTop<scrollviewHeight-height){entriesTop=scrollviewHeight-height;if(!inThePresent)getEntriesNear(false,true);}
$('#entries').css('top',entriesTop+'px');return false;}
function doScroll(delta,delay,auto){clearInterval(scrollInterval);scrollDelta=delta;scrollDelay=delay;doAutoScroll();if(auto){scrollInterval=setInterval(doAutoScroll,100);}}
function buildBookmarkMenu(){var menuList=[];if(queryStr!=''){menuList[menuList.length]={type:'item',title:'Go To Time',fn:bookmarkHandler,bmtype:'goto'};menuList[menuList.length]={type:'separator'};}
menuList[menuList.length]={type:'item',title:'Add Bookmark',fn:bookmarkHandler,bmtype:'private'};var bookmarks=$.cookie('bookmarks');if(bookmarks){bookmarks=bookmarks.split(',');menuList[menuList.length]={type:'separator'};for(var i=0;bookmarks&&i<bookmarks.length;i++){if(bookmarks[i]!=0){bmfields=bookmarks[i].split('/');menuList[menuList.length]={type:'item',title:bmfields[0],fn:bookmarkHandler,bmtype:'goto',bmdate:bmfields[1]};}}}
return menuList;}
function enlivenEntries(){$('.entry-text').menuPopup([{type:'item',title:'Twitter',fn:twitterHandler}]);$('.entry-date').menuPopup(buildBookmarkMenu(),{selectionRequired:false});makeWorkitemTooltip('.worklist-item');}
function makeWorkitemTooltip(className){$(className).tooltip({delay:0,extraClass:"content",showURL:false,bodyHandler:function(){var msg="Test";var worklist_id=$(this).attr('id').substr(9);$.ajax({type:"POST",async:false,url:'getworkitem.php',data:{'item':worklist_id},dataType:'json',success:function(json){msg=json[0]?'<div class = "head">'+json[0]+'</div>':'';msg+=json[3]?json[3]:'';msg+=json[1]?'<div class = "tip-entry">Who: '+json[1]+'</div>':'';msg+=json[2]?'<div class = "tip-entry">Status: '+json[2]+'</div>':'';if(json[5]){var funded=json[5]==1?'Yes':'No';msg+='<div class = "tip-entry">Funded: '+funded+'</div>';}
if(msg==''){msg='No data available';}},error:function(xhdr,status,err){msg='Data loading error.<br />Please try again.';}});return $('<div>').html(msg);}});}
function getNewSpeaker(newSpeaker){var h6=document.createElement('h6');var span=document.createElement('span');span=$(span);if(newSpeaker[5]>0)span.addClass('away');span.data('last_entry',newSpeaker[2]);span.data('last_login',newSpeaker[3]);span.data('status',newSpeaker[4]);span.data('nickname',newSpeaker[1]);span.context.appendChild(document.createTextNode(newSpeaker[1]));span.append($('<em></em>').addClass('typing-icon'));if(checkPenalty(newSpeaker)){span.addClass('penalized');}
if(newSpeaker[0]==userId){span.addClass('me');}
if(newSpeaker[0]==0){span.addClass('guest');}else{span.addClass('logged');}
h6.className='user'+newSpeaker[0];var ret=$(h6).append(span).css('opacity',getSpeakerOpacity(newSpeaker[2]));return ret;}
function getSpeakerOpacity(lastEntryDate){var opacity;var age=Math.min(currentTime-lastEntryDate,24*60*60);if(age<60*60){opacity=1;}else if(age<4*60*60){age-=60*60;opacity=(1-(age/(3*60*60)))*.5+.5;}else{age-=4*60*60;opacity=(1-(age/(20*60*60)))*.4+.1;}
return opacity;}
var speakerCount=0;function fillSpeakerList(newSpeakerList){var online,updateTimes=false;var combined=combineSpeakerLists(oldSpeakerList,newSpeakerList);if(combined[0]==0){updateTimes=true;}else if(oldSpeakerList.length==0&&newSpeakerList.length>0){online=$('#online-users');for(var i=0;i<newSpeakerList.length;i++){if(speakerHeight>0&&speakerHeight*(i+1)>speakerListHeight)break;speaker=getNewSpeaker(newSpeakerList[i]).css('top',i*speakerHeight);online.append(speaker);if(speakerHeight==0)speakerHeight=speaker.find('span').outerHeight()+4;}}else{updateTimes=true;var combinedList=combined[1];var animateFadeIn=function(){var j=0;online=$('#online-users');for(var i in combinedList){if(combinedList[i][0]==2){speaker=getNewSpeaker(combinedList[i][2]).css({top:combinedList[i][1]*speakerHeight,opacity:getSpeakerOpacity(combinedList[i][2])});online.append(speaker);speaker.hide().fadeIn('slow');}}};var animateReposition=function(){var j=0;var fn=animateFadeIn;for(var i in combinedList){if(combinedList[i][0]==1){$('#online-users .user'+i).animate({top:combinedList[i][1]*speakerHeight},'slow',fn);fn=null;}}
if(fn)animateFadeIn();}
var animateFadeOut=function(){var j=0;var fn=animateReposition;for(var i in combinedList){if(combinedList[i][0]==0){$('#online-users .user'+i).fadeOut('slow',fn).remove();fn=null;}}
if(fn)animateReposition();};animateFadeOut();}
for(var i=0;i<newSpeakerList.length;i++){if(newSpeakerList[i][0]==0)$('#online-users .user'+newSpeakerList[i][0]+' span').text(newSpeakerList[i][1]);$('#online-users .user'+newSpeakerList[i][0]).css('opacity',getSpeakerOpacity(newSpeakerList[i][2]));}
for(var i=0;updateTimes&&i<newSpeakerList.length;i++){var span=$('#online-users .user'+newSpeakerList[i][0]+' span');span.data('nickname',newSpeakerList[i][1]);span.data('last_entry',newSpeakerList[i][2]);span.data('last_login',newSpeakerList[i][3]);span.data('status',newSpeakerList[i][4]);if(newSpeakerList[i][5]>0){span.addClass('away');}else{span.removeClass('away');}
if(checkPenalty(newSpeakerList[i])){span.addClass('penalized');}else{span.removeClass('penalized');}
span.hide().show();}
for(var i=0;i<newSpeakerList.length;i++){if(newSpeakerList[i][0]==userId){continue;}
var el=$('#online-users .user'+newSpeakerList[i][0]);if(newSpeakerList[i][8]==3){el.addClass('typing');}else{el.removeClass('typing');}
if(newSpeakerList[i][8]==2){el.addClass('stopped');}else{el.removeClass('stopped');}}
$('#online-users span').unbind();$('div#online-users h6').css('z-index',3).mouseover(function(e){var obj=$($(this).context.firstChild);var msg='Never';if(obj.data('last_entry')){msg=relativeTime(currentTime-obj.data('last_entry'))+".";}
if(obj.data('status')){msg+=" "+obj.data('status');}
$('#livetip').css({top:e.pageY-8,left:e.pageX+12,'z-index':4000}).html('<div>'+msg+'</div>').show();});$('#online-users-container h6').mouseout(function(){$('#livetip').hide();});$('#online-users span').rightMouseUp(function(e){e.preventDefault();});if(userId!=0){var loggedMenuList=[];loggedMenuList[loggedMenuList.length]={type:'item',title:'Current worklist items',fn:userHandler,actionType:'workitems'};loggedMenuList[loggedMenuList.length]={type:'item',title:'Send to Penalty Box',fn:userHandler,actionType:'penalty'};$('#online-users span.logged').menuPopup(loggedMenuList,{selectionRequired:false,context:true});var guestMenuList=[];guestMenuList[guestMenuList.length]={type:'item',title:'Send to Penalty Box',fn:userHandler,actionType:'penalty'};$('#online-users span.guest').menuPopup(guestMenuList,{selectionRequired:false,context:true});$('#online-users span.logged').click(function(e){var className=e.currentTarget.parentNode.className;var id=className.substring(4);var numId=parseInt(id);showUserInfo(numId);return false;});}
oldSpeakerList=newSpeakerList;}
function finishUpdate(scrolllTo,notify,ping){enlivenEntries();applyPopupBehavior();scrollViewTo(scrolllTo);if(notify)alertNewMessage(ping);}
function getEntriesAt(toDate){var now=new Date();thePresent=(now.getTime()/1000-toDate<=3);if(thePresent&&inThePresent){scrollViewTo('100%');return;}
inThePresent=thePresent;var entries=$("#entries");entries.empty();$('.scroll-pane-throbber').show();$.getJSON("aj.php",{'what':'time','count':50,'time':toDate,'prevnext':'','query':queryStr,'filter':systemMsgs},function(json){$('.scroll-pane-throbber').hide();firstDate=json.firstDate;lastDate=json.lastDate;entries.html(json.html);var scrolllTo='50%';if(inThePresent)scrolllTo='100%'
else if(firstDate==earliestDate)scrolllTo='0%';if(systemDrawer){var drawer=$("#system-drawer");drawer.empty();drawer.append(json.system_html);$('#system-drawer-container').scrollTo('100%');}
finishUpdate(scrolllTo,false);});}
function getEntriesNear(prev,autoScroll){inThePresent=false;if(pendingUpdate)return;else if(prev&&firstDate==earliestDate)return;var entries=$("#entries");var removeChildren=new Array();var children=entries.children();$('.scroll-pane-throbber').show();if(prev){for(var i=100;i<children.length;i++){removeChildren[i-100]=children[i];}
if(children.length>100){lastDate=$(children[99]).find('.entry-date').attr('data')|0;}
$.getJSON("aj.php",{'what':'time','count':50,'time':firstDate,'prevnext':'prev','query':queryStr,'filter':systemMsgs},function(json){$('.scroll-pane-throbber').hide();if(json.count>0){firstDate=json.firstDate;var height=entries.height();height=entries.prepend(json.html).height()-height;entries.css('top',(scrollviewHeight-height));finishUpdate(false,false);}
pendingUpdate=false;});}else{for(var i=0;i<children.length-100;i++){removeChildren[i]=children[i];}
if(i<children.length){firstDate=$(children[i]).find('.entry-date').attr('data')|0;}
$.getJSON("aj.php",{'what':'time','count':50,'time':lastDate,'prevnext':'next','query':queryStr,'filter':systemMsgs},function(json){$('.scroll-pane-throbber').hide();var height=entries.height();if(json.count>0){lastDate=json.lastDate;height=entries.append(json.html).height()-height;if(!autoScroll)entries.css('top',($('#entries').position().top-height));finishUpdate(false,false);}else{inThePresent=true;$('#entries-pending').hide();scrollViewTo('100%');}
pendingUpdate=false;});}
if(removeChildren.length>0){var delta=entries.height();$(removeChildren).remove();delta-=entries.height();if(!prev)entries.css('top',entries.position().top+delta);}
pendingUpdate=true;}
var IDLE=1,STOPPED=2,TYPING=3;function sendLocalTypingStatus(status){if(status!==TYPING&&status!==STOPPED){status=IDLE;}
$.ajax({url:'aj.php',data:{'what':'typing','status':status},dataType:'json',success:function(json){}});}
var setLocalTypingStatus=(function(){var typingStatus=IDLE,timeout=null,msBeforeStopped=10000,msBeforeIdle=10000;return function(val){var self=arguments.callee;if(typingStatus!==val){typingStatus=val;sendLocalTypingStatus(val);}
if(timeout!==null){clearTimeout(timeout);timeout=null;}
if(typingStatus===TYPING){timeout=setTimeout(function(){self(STOPPED);},msBeforeStopped);}else if(typingStatus===STOPPED){timeout=setTimeout(function(){self(IDLE);},msBeforeIdle);}};}());function updateTypingIndicators(data){if(data){$('#online-users h6').each(function(){if(/user(\d+)/.test($(this).attr('class'))){var status=data[RegExp.$1]||1;if(status==TYPING){$(this).addClass('typing');}else{$(this).removeClass('typing');}
if(status==STOPPED){$(this).addClass('stopped');}else{$(this).removeClass('stopped');}}});}}
function updateRelativeTimes()
{$(".entry-date").each(function(){$(this).text(relativeTime(currentTime-($(this).attr('data')|0)));});}
function applyMessagePruning()
{var pruningLimit=scrollviewHeight+messagePruningOffsetPixels;var pruning=[];$('.entry').each(function(entry){if($(this).offset().top>(pruningLimit*-1)){return;}
pruning.push(this);});$(pruning).each(function(entry){$(this).unbind();$(this).remove();});if(pruning.length>0){scrollViewTo('100%');updateEntryDates();}}
function updateEntryDates()
{var entries=$('.entry');if(entries.length==0){return;}
firstDate=$('span.entry-date',entries[0]).attr('data');lastDate=$('span.entry-date',entries[(entries.length-1)]).attr('data');}
function getScrollbarDate(now,pos){var percent,mult,offset,delta;var toDate,avgDate=firstDate+(lastDate-firstDate)/2;if(pos<scrollbarBoxHeight/2){var realNum=pos-scrollbarThumbMin;percent=1-(realNum/(scrollbarRange/2));}else{if(inThePresent)return now;var realNum;if(pos<(scrollbarRange/2)){realNum=pos-12;}else if(pos>(scrollbarRange/2)){realNum=pos+12;}else{realNum=pos;}
...
ldelta=mult;if(percent<1)ldelta*=(((percent*100)%20)/20);if(pos<scrollbarBoxHeight/2){toDate=avgDate-offset-ldelta;}else{toDate=avgDate+offset+ldelta;}
if(toDate<earliestDate)toDate=earliestDate;if(toDate>=now)toDate=now;return toDate|0;}
function initializeFileUploadSupport(){var agent=navigator.userAgent.toLowerCase();if((agent.indexOf('iphone')!=-1)){return;}
new AjaxUpload('#uploadButton',{action:'doajaxfileupload.php',data:{},responseType:'json',name:'attachment',onComplete:function(file,data){if(typeof(data.error)!='undefined'){if(data.error!=''){alert("Error uploading file: "+data.error);}}}});}
function relativeTime(time){var mins=60,hour=mins*60,day=hour*24;var week=day*7,month=day*30,year=day*365;var segments=new Array();segments[5]=(time/year)|0;time%=year;segments[4]=(time/month)|0;time%=month;if(!segments[5]){segments[3]=(time/day)|0;time%=day;if(!segments[4]){segments[2]=(time/hour)|0;time%=hour;if(!segments[3]){segments[1]=(time/mins)|0;time%=mins;if(!segments[2]&&!segments[1]){segments[0]=time;}}}}
var cnt,relTime='';var segnames=['sec','min','hr','day','mnth','yr'];for(var i=segments.length-1;i>=0;i--){if(segments[i]){relTime+=segments[i]+' '+segnames[i];if(segments[i]>1)relTime+='s';relTime+=', ';}}
relTime=relTime.substr(0,relTime.length-2);if(relTime!=''){return relTime+" ago";}else{return"just now";}}
function pausecomp(millis)
{var date=new Date();var curDate=null;do{curDate=new Date();}
while(curDate-date<millis);}
function scrollViewTo(scrolllTo){var height=$('#entries').height();if(scrolllTo=='100%'){$('#entries').css('top',scrollviewHeight-height);}else if(scrolllTo=='50%'){$("#entries").css('top',(scrollviewHeight-height)/2);}else if(scrolllTo=='0%'){$("#entries").css('top',0);}}
function tooltipTime(x){var plural='';var mins=60,hour=mins*60;day=hour*24,week=day*7,month=day*30,year=day*365;if(x>=year){x=(x/year)|0;dformat="yr";}
...
if(x>1)plural='s';return x+' '+dformat+plural+' ago';}
function bookmarkHandler(el,text,menudata){var bmdate=null;if(menudata.bmtype=='private'){var date=new Date();date.setTime(date.getTime()+(7*24*60*60*1000));var markerHTML='<div id="marker" title="Bookmarker">'+'  <form id="markerForm" method="post">'+'    <input type="bmname" name="bmname"/><br />'+'    <input type="submit" name="submit" value="Add Bookmark" />'+'  </form>';'</div>';var marker=$(markerHTML).dialog({modal:true,width:'auto',height:'auto'});$("#markerForm").submit(function(){bmdate=el.attr('data');var bmname=$('#markerForm input[name="bmname"]').val();var bookmarks=$.cookie('bookmarks');if(!bookmarks){$.cookie('bookmarks',bmname+'/'+bmdate+',0,0,0',{path:'/',expires:date});}else{bookmarks=bookmarks.split(',');for(var i=3;i>0;i--){bookmarks[i]=bookmarks[i-1];}
bookmarks[0]=bmname+'/'+bmdate;$.cookie('bookmarks',bookmarks.join(','),{path:'/',expires:date});}
$('.entry-date').menuPopup(buildBookmarkMenu(),{selectionRequired:false});marker.dialog('close');marker.remove();return false;});}
if(menudata.bmtype=='goto'){if(menudata.bmdate){bmdate=menudata.bmdate;}else{bmdate=el.attr('data');}
queryStr='';var entries=$("#entries");entries.empty();$('.scroll-pane-throbber').show();$.getJSON("aj.php",{'what':'time','count':50,'time':bmdate,'filter':systemMsgs},function(json){$('.scroll-pane-throbber').hide();inThePresent=false;firstDate=json.firstDate;lastDate=json.lastDate;entries.append(json.html);finishUpdate('50%',false);});}}
function twitterHandler(el,text,menudata){text=text.substr(0,140);userValue=pwdValue='';if($.cookie('twitterUser'))userValue=' value="'+$.cookie('twitterUser')+'"';if($.cookie('twitterPwd'))pwdValue=' value="'+$.cookie('twitterPwd')+'"';var tweeterHTML='<div id="tweeter" title="LoveMachine Tweeter">'+'  <form id="tweeterForm" method="post">'+'    <table><tr><th>Username:</th><th>Password</th><tr>'+'      <tr><td><input type="input" name="twitterUser"'+userValue+'/></td>'+'      <td><input type="password" name="twitterPwd"'+pwdValue+'/></td></tr></table>'+'    <textarea name="tweet">'+text+'</textarea><br />'+'    <input type="submit" name="submit" value="Send Tweet" />'+'  </form>';'</div>';var tweeter=$(tweeterHTML).dialog({modal:true,width:'auto',height:'auto'});$("#tweeterForm").submit(function(){var data='';data+='user='+$('#tweeterForm input[name="twitterUser"]').val();data+='&pwd='+$('#tweeterForm input[name="twitterPwd"]').val();data+='&status='+$('#tweeterForm textarea').text();$.ajax({type:"POST",url:'tweet.php',data:data,dataType:'json',success:function(json){var message;if(json.error){if(json.error.indexOf('authenticate')>=0){$('#tweeterForm').hide();message='We were unable to send your message.<br />Please check your username and password<br />and try again.';}}else{var date=new Date();date.setTime(date.getTime()+(7*24*60*60*1000));$.cookie('twitterUser',$('#tweeterForm input[name="twitterUser"]').val(),{path:'/',expires:date});$.cookie('twitterPwd',$('#tweeterForm input[name="twitterPwd"]').val(),{path:'/',expires:date});$('#tweeterForm').hide();message='Congratulations!<br /><br/>Your tweet has been sent!';}
$('#tweeter').append(message+'<br /><br />');$('#tweeter').append('<input id="errorok" type="submit" name="ok" value="Ok" />').click(function(){tweeter.dialog('close');tweeter.remove();return false;});},error:function(xhdr,status,err){message='We were unable to send your message. An error occured while communicating with twitter.<br />Please try again.';$('#tweeter').append(message+'<br /><br />');$('#tweeter').append('<input id="errorok" type="submit" name="ok" value="Ok" />').click(function(){tweeter.dialog('close');tweeter.remove();return false;});}});return false;});return false;}
function userHandler(el,text,menudata){if(menudata.actionType=='penalty'){onPenaltyBox(el);}
if(menudata.actionType=='workitems'){onUserItems(el);}}
function updateScrollbarThumb(e,reset){var targetPos=Math.min(scrollbarThumbMax,Math.max(scrollbarThumbMin,e.pageY-$('.scrollbar-box').offset().top));var now=new Date();var dte=now;now=(now.getTime()/1000)|0;var toDate=getScrollbarDate(now,targetPos);if(!reset){var text=relativeTime(now-toDate);$('.scrollbar-thumb-text').text(text);if(text.length>0){var width=$('.scrollbar-thumb-text').outerWidth();$('.scrollbar-thumb').css({'top':targetPos-12,'width':width+15,'left':-width,'background-position':(width-225)+'px 0px'});}}else{$('span.scrollbar-thumb-text').html('&nbsp;');$('.scrollbar-thumb').stop().animate({top:((scrollbarBoxHeight-scrollbarThumbHeight)/2)+'px'},{queue:false,duration:1000,easing:'easeOutBounce'});$('.scrollbar-thumb').css({'background-position':'-225px 0px','left':0,'width':15});}
return toDate;}
function relativity(){IEFix();scrollviewHeight=$('.scroll-view').outerHeight();scrollviewTop=$('.scroll-view').offset().top;scrollbarBoxHeight=$('.scrollbar-box').outerHeight();scrollbarThumbHeight=$('.scrollbar-thumb').outerHeight();$('.scrollbar-thumb').css('top',((scrollbarBoxHeight-scrollbarThumbHeight)/2)+'px');scrollbarThumbMin=scrollbarThumbHeight/2;scrollbarThumbMax=scrollbarBoxHeight-scrollbarThumbHeight/2;scrollbarRange=scrollbarThumbMax-scrollbarThumbMin;scrollViewTo('100%',true);speakerListHeight=$('#online-users').outerHeight();}
$(window).resize(function(){relativity();});function initSound()
{soundManager.url='flash/soundmanager2.swf';soundManager.debugMode=false;soundManager.onready(function(){chatSound=soundManager.createSound({id:'chatSound',url:'mp3/bubblepop.mp3',autoLoad:true,autoPlay:false});systemSound=soundManager.createSound({id:'systemSound',url:'mp3/plazzle.mp3',autoLoad:true,autoPlay:false});botSound=soundManager.createSound({id:'botSound',url:'mp3/sweosh.mp3',autoLoad:true,autoPlay:false});pingSound=soundManager.createSound({id:'pingSound',url:'mp3/warble.mp3',autoLoad:true,autoPlay:false});});}
$(window).ready(function(){IEFix();$('.scroll-pane-throbber').hide();$('#guideline').show();$('#footer').show();applyPopupBehavior();initializeFileUploadSupport();initSound();$('#user-info').dialog({autoOpen:false,modal:false,height:500,width:800});$("div#loginbox").hide();$("div#loginbox").addClass('loginexpanded');$("div#Audiosettings").addClass('audioboxexpanded');$.get("login.php",function(response){$("div#loginbox").html($("#in-lt",response).html());$('#username').watermark('Email address',{useNative:false});$('#password').watermark('Password',{useNative:false});});$("#loginlink").bind('click',function(event,ui){if($('div#loginbox').is(":visible")){$('div#loginbox').hide('drop',{direction:'left'},500);}else{$('div#loginbox').show('drop',{direction:'left'},500);}
event.preventDefault();});$("#audioSwitch").bind('click',function(event,ui){if($('div#Audiosettings').is(":visible")){$('div#Audiosettings').hide('drop',{direction:'left'},500);}else{$('div#Audiosettings').show('drop',{direction:'left'},500);}
event.preventDefault();});relativity();$('.scrollbar-thumb').mousedown(function(e){var mouseTrigger=true;var offset=$('.scrollbar-thumb').offset();$('.scrollbar-thumb').css({'width':'200px'});updateScrollbarThumb(e,false);$(document).mousemove(function(e){mouseTrigger=false
updateScrollbarThumb(e,false);});$(document).mouseup(function(e){$(document).unbind('mousemove').unbind('mouseup');var toDate=updateScrollbarThumb(e,true);getEntriesAt(toDate);});});$('.scrollbar-box').click(function(e){if(mouseTrigger){mouseTrigger=false;return;}
var targetPos=e.pageY-$('.scrollbar-box').offset().top;if(e.pageX-$('.scrollbar-box').offset().left>15)return;getEntriesNear(targetPos<scrollbarBoxHeight/2,false);});$('.scrollbar-up').mousedown(function(){doScroll(20,4,true);$(document).mouseup(function(){clearInterval(scrollInterval);});return false;});$('.scrollbar-up').click(function(){doScroll(20,0,false);return false;});$('.scrollbar-down').mousedown(function(){doScroll(-20,4,true);$(document).mouseup(function(){clearInterval(scrollInterval);});return false;});$('.scrollbar-down').click(function(){doScroll(-20,0,false);return false;});$('.scroll-view').bind('mousewheel',function(event,delta){delta=delta||(event.wheelDelta?event.wheelDelta/120:(event.detail)?-event.detail/3:0);doScroll(delta*12,false);});$(document).keydown(function(e){switch(e.keyCode){case 33:getEntriesNear(true);return false;case 34:getEntriesNear(false);return false;case 38:doScroll(20,0,true);$(document).keyup(function(){clearInterval(scrollInterval);});return false;case 40:doScroll(-20,0,true);$(document).keyup(function(){clearInterval(scrollInterval);});return false;}});$('#go').click(function(){showLatest();});$("#search").click(function(){$("#searchForm").submit();return false;});$("#search_reset").click(function(){if(queryStr.length>0){showLatest();}
return false;});$("#searchForm").submit(function(){queryStr=$('#query').val();inThePresent=false;var entries=$("#entries");entries.empty();$('.scroll-pane-throbber').show();$.getJSON("aj.php",{'what':'latest','count':50,'query':queryStr,'filter':systemMsgs},function(json){inThePresent=false;$('.scroll-pane-throbber').hide();firstDate=json.firstDate;lastDate=json.lastDate;entries.append(json.html);finishUpdate('100%',false);if(systemDrawer){var drawer=$("#system-drawer");drawer.empty();drawer.append(json.system_html);$('#system-drawer-container').scrollTo('100%');}});return false;});$("#sub").click(function(){if(!penalized){sendEntry();}
return false;});$("#message-pane").keyup(function(e){if(e.keyCode==16){shiftPressed=false;}});$("#message-pane").keydown(function(e){if(e.keyCode==16){shiftPressed=true;}
if(e.keyCode==13){if(shiftPressed){return true;}
sendEntry();return false;}});$("#message-pane").keydown(function(e){if(e.keyCode==38||e.keyCode==40){e.stopPropagation();}});$('<div id="livetip"></div>').hide().appendTo('body');if(gotoDate!=0){getEntriesAt(gotoDate);}else{getLatestEntries(1);scrollViewTo('100%',true);}
for(timer in $.timer)
{clearInterval(timer);}
$.timer['getLatestEntries']=setInterval('getLatestEntries()',pollingInterval);$.timer['getLatestSpeakers']=setInterval('getLatestSpeakers()',speakerPollingInterval);scrollViewTo('100%',true);enlivenEntries();$.get("aj.php",{'what':'speakeronline','author_id':userId});speakerListHeight=$('#online-users').height();getLatestSpeakers();$('img#drawer-switch').click(function(){if(!penalized){if(!systemDrawer){openDrawer();}else{closeDrawer();}
showLatest();}});$('#popup-user-info #roles input[type="submit"]').click(function(){var name=$(this).attr('name');switch(name){case"cancel":$('#popup-user-info').dialog('close');return false;}});AudioOn();});function openDrawer(){$('.scroll-pane').addClass('shrinked-pane');$('#system-drawer-container').show();systemDrawer=true;systemMsgs='system';$('#drawer-switch').css('background-position','-150px -88px');}
function closeDrawer(){$('#system-drawer-container').hide();$('.scroll-pane').removeClass('shrinked-pane');systemDrawer=false;systemMsgs='all';$('#drawer-switch').css('background-position','-66px -88px');IEFix();}
$(window).unload(function(){$.get("aj.php",{'what':'speakeroffline','author_id':userId});pausecomp(400);});function AjaxPopup(popupId,titleString,urlString,keyId,fieldArray,successFunc)
{$(popupId).data('title.dialog',titleString);$.ajax({type:"POST",url:urlString,data:'item='+keyId,dataType:'json',success:function(json){$.each(fieldArray,function(key,value){if(value[0]=='input'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form input[name="'+value[1]+'"]').val(eval(value[2]));}else{$('.popup-body form input[name="'+value[1]+'"]').val(value[2]);}}
if(value[0]=='textarea'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form textarea[name="'+value[1]+'"]').val(eval(value[2]));}else{$('.popup-body form textarea[name="'+value[1]+'"]').val(value[2]);}}
if(value[0]=='checkbox'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form checkbox[name="'+value[1]+'"] option[value="'+eval(value[2])+'"]').attr('checked','checked');}else{$('.popup-body form checkbox[name="'+value[1]+'"] option[value="'+value[2]+'"]').attr('checked','checked');}}
if(value[0]=='span'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form '+value[1]).text(eval(value[2]));}else{$('.popup-body form '+value[1]).text(value[2]);}}});if(successFunc!==undefined){successFunc(json);}}});}
function SimplePopup(popupId,titleString,keyId,fieldArray,successFunc)
{$(popupId).data('title.dialog',titleString);$.each(fieldArray,function(key,value){if(value[0]=='input'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form input[name="'+value[1]+'"]').val(eval(value[2]));}else{$('.popup-body form input[name="'+value[1]+'"]').val(value[2]);}}
if(value[0]=='textarea'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form textarea[name="'+value[1]+'"]').val(eval(value[2]));}else{$('.popup-body form textarea[name="'+value[1]+'"]').val(value[2]);}}
if(value[0]=='checkbox'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form checkbox[name="'+value[1]+'"] option[value="'+eval(value[2])+'"]').attr('checked','checked');}else{$('.popup-body form checkbox[name="'+value[1]+'"] option[value="'+value[2]+'"]').attr('checked','checked');}}
if(value[0]=='span'){if(value[3]!=undefined&&value[3]=='eval'){$('.popup-body form '+value[1]).text(eval(value[2]));}else{$('.popup-body form '+value[1]).text(value[2]);}}});if(successFunc!==undefined){successFunc(json);}}
function showUserInfo(userId){$('#user-info').html('<iframe id="modalIframeId" width="100%" height="100%" marginWidth="0" marginHeight="0" frameBorder="0" scrolling="auto" />').dialog('open');$('#modalIframeId').attr('src',worklistUrl+'/userinfo.php?id='+userId);return false;}
function IEFix(){var p=document.getElementsByTagName('body')[0];var ih=0;var iw=0;if(typeof window.innerWidth!='undefined'){ih=window.innerHeight;iw=window.innerWidth;}else if(typeof document.documentElement){ih=document.documentElement.clientHeight;iw=document.documentElement.clientWidth;}else{}
var base=(ih-$('#footer').outerHeight());$('#footer').css('top',(base)+'px').css('position','fixed').css('width','100%').css('left',0);if(iw>902)
var head_h=($('#head')[0].offsetHeight)+20;else if(iw>650)
var head_h=($('#head')[0].offsetHeight)+50;else
var head_h=($('#head')[0].offsetHeight)+80;$('#guideline').css('height',(Number(base)-head_h)+'px');$('.scrollbar').css('height',(Number(base)-(head_h-2))+'px');$('#online-users-container').css('height',(Number(base)-head_h)+'px').css('overflow','hidden').css('padding',0);$('div.scrollbar-hold').css('height',$('#guideline').outerHeight()-28).css('top','15px');}
$(function(){if($('#status-lbl').find('b').html()==""||$('#status-lbl').find('b').html()==" "){$('#status-update').show();$("#status-share").show();}else
$('#status-lbl').show();$("#status-update").focus(function(){$("#status-share").show();});$("#status-lbl").mouseenter(function(){$('#status-lbl').hide();$('#status-update').show();$('#status-share').show();});$("#status-wrap").mouseleave(function(){if($('#status-lbl').find('b').html()!=""){$('#status-update').hide();$("#status-share").hide();$('#status-lbl').show();}});$("#status-share-btn").click(function(){if($("#status-update").val()=="What are you working on?")
$("#status-update").val("");$.ajax({url:"update_status.php",type:"POST",data:"action=update&status="+$("#status-update").val(),dataType:"text",success:function(){if($("#status-update").val()!=""||$("#status-update").val()!=" "){$('#status-update').hide();$('#status-lbl').show();$("#status-share").hide();}
$('#status-lbl').html('<b>'+$("#status-update").val()+'</b>');}});return false;});});(function(){var d=document,w=window;function get(element){if(typeof element=="string")
...
return form;}};})();function fancyAlert(titleString,message){$('body').append('<div id="message_container">'+'</div>');$('#message_container').attr('title',titleString);$('#message_container').html(message);$('#message_container').dialog({modal:true,buttons:{Ok:function(){$(this).dialog('destroy');}},close:function(){$(this).dialog('destroy');$('#message_container').remove();}});}
if($.ui){$.widget('ui.toaster',{_init:function(){var self=this;var wrapper='#ui-toaster-'+self.options.position;if(!$(wrapper).get(0)){$('<div />').attr('id','ui-toaster-'+self.options.position).appendTo('body');}
self.toaster=$('<div style="display:none;" class="ui-toaster" />').append($('<span class="ui-toaster-border-tr" /><span class="ui-toaster-border-tl" /><span class="ui-toaster-border-tc" />')).append($('<span class="ui-toaster-body" />').html($('<div />').append($(self.element).html()))).append($('<span class="ui-toaster-border-br" /><span class="ui-toaster-border-bl" /><span class="ui-toaster-border-bc" />')).width(self.options.width).hover(function(){self.pause.apply(self)},function(){self.resume.apply(self)})
[(self.options.position.match(/bl|br/))?'prependTo':'appendTo'](wrapper);if(self.options.closable){self.toaster.addClass('ui-toaster-closable');if($(self.toaster).find('.ui-toaster-close').length>0){$('.ui-toaster-close',$(self.toaster)).click(function(){self.hide.apply(self);});}
else{$(self.toaster).click(function(){self.hide.apply(self);});}}
...
effect.apply(self.toaster,[self.options.speed,function(){if(self.options.onClosed)self.options.onClosed.apply(self.toaster);$(self.toaster).remove();}]);},resume:function(){var self=this;self.timer=setTimeout(function(){self.close.apply(self);},self.options.timeout*1000+self.options.delay*1000);},pause:function(){clearTimeout(this.timer);}});$.ui.toaster.defaults={delay:0,timeout:3,width:200,position:'br',speed:'slow',closable:true,sticky:false,onClose:false,onClosed:false,onOpen:false,onOpened:false,onHide:false,show:$.fn.slideDown,hide:$.fn.fadeOut,close:$.fn.slideUp};}
$(window).ready(function(){$('.slide-out-div').show();$('.slide-out-div').tabSlideOut({tabHandle:'.handle',pathToTabImage:'images/sprite.png',imageHeight:'22px',imageWidth:'82px',tabLocation:'top',speed:300,action:'click',topPos:'150px',leftPos:(function(){return Math.round(($('body').innerWidth()-$('.slide-out-div').width())/2);})(),fixedPosition:false});$('.handle').css({'text-align':'center','text-indent':'0px','line-height':'22px','font-family':'Arial, Helvetica, Sans-serif','font-size':'13px','font-weight':'bold','background-position':'-126px -36px','color':'#444'});$('#feedback-submit').click(function(e){var email=$('#feedback-email').val();var message=$('#feedback-message').val();if(message!=''){$('.LV_validation_message').remove();$('.handle').click();$('#feedback-message').val('');$.post('feedback.php',{message:message,email:email},function(){$('<div><p>Your feedback was sent.<br />Thank you!</p></div>').toaster({position:'tr',timeout:3,show:$.fn.fadeIn,close:$.fn.fadeOut});});}
return false;});});$(window).ready(function(){$('#popup-penalty').dialog({autoOpen:false});$('#popup-guest-selector').dialog({autoOpen:false});$('#popup-penalty #penalty input[name=penalize-cancel]').click(function(){$('#popup-penalty').dialog('close');return false;});});function onPenaltyBox(el){var id=el.parent().attr('class').substring(4);if(id!=0){$('#penalty #nicknamed').html(el.data('nickname'));showPenaltyDialog(id);}else{$('#penalty #nicknamed').html('Guest');$.getJSON("penaltyajax.php",{'what':'guestlist'},function(json){$('#popup-guest-selector #guest-list .guest-entry').remove();for(var i=0;i<json.length;i++){var div=$('<div>');div.addClass('guest-entry');div.data('ip',json[i].ip);div.append(json[i].entry);$('#popup-guest-selector #guest-list').append(div);}
$('#popup-guest-selector').dialog('open');$('#popup-guest-selector #guest-list .guest-entry').click(function(){$('#popup-guest-selector').dialog('close');var ip=$(this).data('ip');showPenaltyDialog(0,ip);});});}}
function showPenaltyDialog(penaltyId,penaltyIp){penaltyIp=penaltyIp!=undefined?penaltyIp:0;$.getJSON("penaltyajax.php",{'what':'checkpenalize','penalizer_id':userId,'penalated_id':penaltyId,'penalated_ip':penaltyIp},function(json){switch(json.status){case'CAN_PENALIZE':if(penaltyId!=0){$('#popup-penalty').data('title.dialog','Send User to Penalty Box');}else{$('#popup-penalty').data('title.dialog','Send Guest to Penalty Box');}
$('#popup-penalty #penalty input[name=penalize]').unbind();$('#popup-penalty #penalty input[name=penalize]').click(function(){var reason=$('#popup-penalty #penalty textarea[name=penalty-reason]').val();$.post("penaltyajax.php",{'what':'penalize','penalizer_id':userId,'penalated_id':penaltyId,'penalated_ip':penaltyIp,'reason':reason});$('#popup-penalty').dialog('close');return false;});$('#popup-penalty #penalty textarea[name=penalty-reason]').val('');$('#popup-penalty').dialog('open');break;case'ALREADY_PENALIZED':fancyAlert("Already Penalized","You already penalized this user. Can't do it again until user goes into Penalty Box");break;case'ALREADY_SUSPENDED':fancyAlert("Already Suspended","User is already suspended");break;case'IN_BOX':fancyAlert("User in Penalty Box","User is already in penalty box");break;case'CANT_PENALIZE':fancyAlert("Can't Penalize","You've been to Penalty Box more than 2 times - you can't penalize other users!");break;}});}
function checkPenalty(speaker){if(speaker[0]==0&&userId==0){for(var i=0;i<speaker[5].length;i++){if(speaker[5][i].ip==userIp&&speaker[5][i].timeleft>0){suspended=speaker[5][i].suspended==1?true:false;fillPenaltyDescriptions(0,userIp);openPenaltyBox(speaker[5][i].timeleft);}}
return false;}else{if(speaker[0]!=0){if(speaker[6]>0||speaker[7]==1){if(speaker[0]==userId){suspended=speaker[7]==1?true:false;fillPenaltyDescriptions(userId);openPenaltyBox(speaker[6]);}
return true;}else{return false;}}
return false;}}
function openPenaltyBox(timeout){penalized=true;$('#penalty-container').show();openDrawer();systemMsgs='all'
$('#system-drawer').empty();$('#sub').attr('disabled','disabled');$('#message').attr('disabled','disabled');$('input[name="attachment"]').attr('disabled','disabled');if(suspended){$('#penalty-countdown').html('Your account has been suspended!');}else{$('#penalty-countdown').countdown({until:+timeout,format:'MS',onExpiry:closePenaltyBox});}}
function closePenaltyBox(){penalized=false;$('#penalty-container').hide();closeDrawer();$('#sub').attr('disabled','');$('#message').attr('disabled','');$('input[name="attachment"]').attr('disabled','');}
function fillPenaltyDescriptions(penaltyId,penaltyIp){penaltyIp=penaltyIp!=undefined?penaltyIp:0;$.getJSON("penaltyajax.php",{'what':'getreasons','penalizer_id':userId,'penalated_id':penaltyId,'penalated_ip':penaltyIp},function(json){$('#penalty-descriptions .penalty-description-entry').remove();for(var i=json.length-1;i>=0;i--){if(json[i].reason!=''){var div=$('<div>');div.addClass('penalty-description-entry');div.html(json[i].reason);div.append('<br /><div class = "given">'+json[i].time+'</div><div style="clear:both; border: none;" />');$('#penalty-descriptions').append(div);}}});}
$(window).ready(function(){$('#popup-useritems').dialog({autoOpen:false});});function onUserItems(el){var id=el.parent().attr('class').substring(4);$.getJSON('getuseritems.php?id='+id,function(json){if(json.length>0){$('#popup-useritems #item-list .item-entry').remove();for(var i=0;i<json.length;i++){var div=$('<div>');div.addClass('item-entry');div.attr('id','worklist-'+json[i].id);div.append('<div class = "item-head">#'+json[i].id+' - '+json[i].summary+'</div>');var warn='';if(json[i].future_delta<0){warn=' class = "warn"';}
div.append('<div class = "item-body"><div class = "done-in">Done in: <span'+warn+'>'+json[i].relative+'</span></div><div class = "clear"></div></div>');div.data('id',json[i].id);$('#popup-useritems #item-list').append(div);}
makeWorkitemTooltip('.item-entry');$('.item-entry').click(function(){window.open(worklistUrl+'/workitem.php?job_id='+$(this).data('id')+'&action=view');if(json.length==1){$('#popup-useritems').dialog('close');}});$('#popup-useritems #nicknamed').html(el.data('nickname'));$('#popup-useritems').dialog('open');}else{fancyAlert("No current worklist items",el.data('nickname')+" is not working on any worklist items at the moment");}});}
function TypingNotifier(localUser){var self=this;var customEventType='userkbd';$.extend(this,{outgoingNotification:$('<span></span>'),statusChanged:$('<span></span>'),localDispatchDelay:3000,localAutoStopDelay:10000,localAutoIdleDelay:20000,remoteAutoStopDelay:15000,remoteAutoIdleDelay:35000,_dispatchTimeout:undefined,_statusToDispatch:undefined,__dispatchStatus:function(){if(!this._dispatchTimeout&&this._statusToDispatch){this.outgoingNotification.trigger(customEventType,{local:true,user:localUser,status:this._statusToDispatch});this._statusToDispatch=undefined;var self=this;this._dispatchTimeout=setTimeout(function(){self._dispatchTimeout=undefined;self.__dispatchStatus();},this.localDispatchDelay);}},_dispatchStatus:function(value){this._statusToDispatch=value;this.__dispatchStatus();},_status:{},_statusChangeTimeout:{},isLocal:function(user){return user===localUser;},_scheduleStatusChange:function(user,value,delay){var self=this;this._cancelStatusChange(user);this._statusChangeTimeout[user]=setTimeout(function(){self._statusChangeTimeout[user]=undefined;self._setStatus(user,value);},delay);},_cancelStatusChange:function(user){if(this._statusChangeTimeout[user]){clearTimeout(this._statusChangeTimeout[user]);this._statusChangeTimeout[user]=undefined;}},getLocalUserStatus:function(){return this._status[localUser]||TypingNotifier.IDLE;},getStatus:function(user){return this._status[user]||TypingNotifier.IDLE;},_setStatus:function(user,value){if(this.isLocal(user)){this._dispatchStatus(value);}
this.statusChanged.trigger(customEventType,{local:this.isLocal(user),user:user,status:value});if(value===TypingNotifier.IDLE){delete this._status[user];}else{this._status[user]=value;}
switch(value){case TypingNotifier.TYPING:this._scheduleStatusChange(user,TypingNotifier.STOPPED,this.isLocal(user)?this.localAutoStopDelay:this.remoteAutoStopDelay);break;case TypingNotifier.STOPPED:this._scheduleStatusChange(user,TypingNotifier.IDLE,this.isLocal(user)?this.localAutoIdleDelay:this.remoteAutoIdleDelay);break;case TypingNotifier.IDLE:this._cancelStatusChange(user);break;}},setLocalUserStatus:function(value){this._setStatus(localUser,value);},setRemoteUserStatus:function(user,value){if(user!==localUser){this._setStatus(user,value);}}});}
TypingNotifier.TYPING='typing';TypingNotifier.STOPPED='stopped';TypingNotifier.IDLE='idle';function SLEmail(B,C){if(B)B=B.replace(/^\s+|\s+$/g,"");Validate.Email(B,C);}
function SLEmail2(B,C){if(B&&B.replace(/^\s+|\s+$/g,"").match(/^[\w\d]+ \([\w\d._%-]+@[\w\d.-]+\.[\w]{2,4}\)$/))return true;return Validate.Email(B,C);}
var LiveValidation=function(B,A){this.initialize(B,A);};LiveValidation.VERSION="1.3 standalone";LiveValidation.TEXTAREA=1;LiveValidation.TEXT=2;LiveValidation.PASSWORD=3;LiveValidation.CHECKBOX=4;LiveValidation.SELECT=5;LiveValidation.FILE=6;LiveValidation.massValidate=function(C){var D=true;for(var B=0,A=C.length;B<A;++B){var E=C[B].validate();if(D){D=E;}}return D;};LiveValidation.prototype={validClass:"LV_valid",invalidClass:"LV_invalid",messageClass:"LV_validation_message",validFieldClass:"LV_valid_field",invalidFieldClass:"LV_invalid_field",initialize:function(D,C){var A=this;if(!D){throw new Error("LiveValidation::initialize - No element reference or element id has been provided!");}
