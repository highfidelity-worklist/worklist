...
if( $active == 'FALSE' ) {
    ...
    FROM `".USERS."` 
    LEFT JOIN (SELECT `mechanic_id`, COUNT(`mechanic_id`) AS `count` FROM `".WORKLIST."` WHERE (`status`!='SUGGESTED' AND `status`!='PASS' AND `status`!='SUGGESTEDwithBID' AND `status`!='BUG' AND `status`!='BIDDING' AND `status`!='') GROUP BY `mechanic_id`) AS `mechanics` ON `".USERS."`.`id` = `mechanics`.`mechanic_id` 
    LEFT JOIN (SELECT `creator_id`, COUNT(`creator_id`) AS `count` FROM `".WORKLIST."` WHERE (`status`!='SUGGESTED' AND `status`!='PASS' AND `status`!='SUGGESTEDwithBID' AND `status`!='BUG' AND `status`!='BIDDING' AND `status`!='') AND (`creator_id` != `mechanic_id`) GROUP BY `creator_id`) AS `creators` ON `".USERS."`.`id` = `creators`.`creator_id` 
