...
class CURLHandler {
    ...
    public static function doRequest($method, $url, $vars = array(), $auth = false, $login = false, $async = false){
        ...
        if ($login){
            curl_setopt_array($curlInterface, array(CURLOPT_URL => $url, CURLOPT_SSLVERSION => 4, CURLOPT_SSL_VERIFYPEER => FALSE, CURLOPT_SSL_VERIFYHOST => 2, CURLOPT_CONNECTTIMEOUT => 5, CURLOPT_RETURNTRANSFER => 1, CURLOPT_FOLLOWLOCATION => 1, CURLOPT_HEADER => 0));
        } else {
            curl_setopt_array($curlInterface, array(CURLOPT_URL => $url, CURLOPT_SSLVERSION => 4, CURLOPT_SSL_VERIFYPEER => FALSE, CURLOPT_SSL_VERIFYHOST => 2, CURLOPT_CONNECTTIMEOUT => 5, CURLOPT_RETURNTRANSFER => 0, CURLOPT_FOLLOWLOCATION => 1, CURLOPT_HEADER => 0));
        }
            curl_setopt_array($curlInterface, array(CURLOPT_URL => $url, CURLOPT_SSLVERSION => 4, CURLOPT_SSL_VERIFYPEER => FALSE, CURLOPT_SSL_VERIFYHOST => 2, CURLOPT_CONNECTTIMEOUT => 5, CURLOPT_RETURNTRANSFER => 1, CURLOPT_FOLLOWLOCATION => 1, CURLOPT_HEADER => 0));
