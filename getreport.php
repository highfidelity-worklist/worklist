...
$qcnt  = "SELECT count(*)";
$qsel = "SELECT `worklist_id`,`summary`,`desc`,`status`,`".USERS."`.`nickname` as `payee`,`".FEES."`.`amount`";
$qbody = "FROM `".FEES."`
...
{
    $report[$i] = array($row['worklist_id'], $row['summary'], $row['desc'], $row['status'], $row['payee'], $row['amount']);
}
